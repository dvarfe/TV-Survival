...
class SurvPredictor:
    ...
    def predict(self, dataloader: DataLoader, times: np.ndarray = np.arange(0, 10)) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def get_expected_time(self, dataloader: DataLoader, times: np.ndarray = np.arange(0, 10)) -> Tuple[np.ndarray, pd.DataFrame]:

...
def calculate_metrics(model, predictions_aggregator, scorer, method, weight, n_samples, timeshift, X, X_gt, metric_postfix='test', metrics_list=['ci', 'ibs'], df_train_gt=None):
...
def eval_model(data_path, model_name, agg_dict, metrics_list, metric_postfix='test'):
    """
            ...
            for method in agg_dict:
                for weight in agg_dict[method]:
                    ...
                    result_row = calculate_metrics(model, agg_dict[method][weight], scorer, method, weight, n_samples,
                                                   cur_timeshift, cur_X_pred, cur_X_gt,
